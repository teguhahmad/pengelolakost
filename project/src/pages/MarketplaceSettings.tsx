...
import AmenitiesForm from '../components/marketplace/AmenitiesForm';
...
const MarketplaceSettings: React.FC = () => {
  ...
  const [showCommonAmenitiesForm, setShowCommonAmenitiesForm] = useState(false);
  const [showParkingAmenitiesForm, setShowParkingAmenitiesForm] = useState(false);
  ...
  const handleSaveCommonAmenities = async (amenities: string[]) => {
    try {
      setIsLoading(true);
      setError(null);
      ...
      setShowCommonAmenitiesForm(false);
      setShowSuccess(true);
      setTimeout(() => setShowSuccess(false), 3000);
    } catch (err) {
      console.error('Error saving common amenities:', err);
      setError('Failed to save common amenities');
    } finally {
      setIsLoading(false);
    }
  };

  const handleSaveParkingAmenities = async (amenities: string[]) => {
    try {
      setIsLoading(true);
      setError(null);
      ...
      setShowParkingAmenitiesForm(false);
      setShowSuccess(true);
      setTimeout(() => setShowSuccess(false), 3000);
    } catch (err) {
      console.error('Error saving parking amenities:', err);
      setError('Failed to save parking amenities');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    ...
    >
      <div className="space-y-6">
                  ...
                  <div>
                    <div className="flex justify-between items-center mb-4">
                      ...
                      <Button
                        variant="outline"
                        size="sm"
                        onClick={() => setShowCommonAmenitiesForm(true)}
                      >
                        Edit Fasilitas
                      </Button>
                    </div>
                    <div className="flex flex-wrap gap-2">
                      {settings.common_amenities.length > 0 ? (
                  ...
                  <div>
                    <div className="flex justify-between items-center mb-4">
                      ...
                      <Button
                        variant="outline"
                        size="sm"
                        onClick={() => setShowParkingAmenitiesForm(true)}
                      >
                        Edit Fasilitas
                      </Button>
                    </div>
                    <div className="flex flex-wrap gap-2">
                      {settings.parking_amenities.length > 0 ? (
        ...
        {showCommonAmenitiesForm && (
          <AmenitiesForm
            title="Edit Fasilitas Umum"
            amenities={settings.common_amenities}
            suggestions={COMMON_AMENITIES_SUGGESTIONS}
            onSave={handleSaveCommonAmenities}
            onClose={() => setShowCommonAmenitiesForm(false)}
          />
        )}

        {showParkingAmenitiesForm && (
          <AmenitiesForm
            title="Edit Fasilitas Parkir"
            amenities={settings.parking_amenities}
            suggestions={PARKING_AMENITIES_SUGGESTIONS}
            onSave={handleSaveParkingAmenities}
            onClose={() => setShowParkingAmenitiesForm(false)}
          />
        )}
