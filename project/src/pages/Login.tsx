...
import { Mail, Lock, AlertCircle, User, LogIn } from 'lucide-react';
...
const Login: React.FC = () => {
  ...
  const checkPropertyManagerAccess = async (userId: string): Promise<boolean> => {
    try {
      ...
      if (!backofficeError && backofficeUsers && backofficeUsers.length > 0) {
        const backofficeUser = backofficeUsers[0];
        if (backofficeUser.status === 'active' && backofficeUser.role === 'superadmin') {
          return true;
        }
      }

      // If not superadmin, check if they own any properties
      ...
      console.error('Error checking property manager access:', err);
      return false;
    }
  };
  ...
  const handleAuth = async (e: React.FormEvent) => {
    ...
    try {
      if (isLogin) {
        ...
        // Check if user has property manager access
        const hasAccess = await checkPropertyManagerAccess(user.id);
        if (!hasAccess) {
          throw new Error('Anda tidak memiliki izin sebagai pengelola properti.');
        }
