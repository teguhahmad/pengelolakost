...
const Header: React.FC<HeaderProps> = ({ title, onMenuClick, onNavigate }) => {
  ...
  // Load data pengguna dan subscribe ke chat
  useEffect(() => {
    ...
    // Optional: gunakan authStateChange untuk memantau login/logout
  ...
  // Subscribe ke perubahan pesan chat
  const subscribeToUnreadChats = async () => {
    const result = await supabase.auth.getUser();
    if (result.error || !result.data?.user) {
      console.error('User not authenticated:', result.error);
      return;
    }
